...
This script will deal with calculating alpha and beta diversity for 16S and ITS 
...
Now I can continue analyzing each experiment independently
####Richness
```{r}
...
##2.Chao1
chao <- lapply(richness, function(df){
        lm(df$Chao1 ~ plant_pathway_induced + libsize_after_filtering,
                data=df)})

chao.table <- lapply(chao, function(model){
  car::Anova(model, type="III", test.statistic="F")#using type III, as its unbalanced
})

#check if assumptions are met
lapply(chao, function(model){simulateResiduals(model, plot=T)}) #ok
lapply(chao, function(model){hist(resid(model))}) #normalish
lapply(chao, function(model){qqnorm(resid(model))}) #also

#emmeans: Post-hoc test
lapply(chao, function(model){plot(emmeans(model, pairwise ~  plant_pathway_induced))}) 
chao.emmeans <- lapply(chao, function(model){
              emmeans(model, pairwise ~  plant_pathway_induced)})
  
chao.df <- lapply(chao.emmeans, function(emmeans){
  emmeans$contrasts %>% summary(infer=TRUE) %>% as.data.frame()})

```

Difference between control and JA only in Phytohormones experiment.
...
##4.Simpson
simp <- lapply(richness, function(df){
        lm(df$Simpson ~ plant_pathway_induced + libsize_after_filtering,
                data=df)})

simp.table <- lapply(simp, function(model){
  car::Anova(model, type="III", test.statistic="F")#using type III, as its unbalanced
})

simp.table #both signigicant

#check if assumptions are met
lapply(simp, function(model){simulateResiduals(model, plot=T)}) # not ok for phyto
lapply(simp, function(model){hist(resid(model))}) #normalish
lapply(simp, function(model){qqnorm(resid(model))}) #also

#emmeans: Post-hoc test
lapply(simp, function(model){plot(emmeans(model, pairwise ~  plant_pathway_induced))}) 
simp.emmeans <- lapply(simp, function(model){
              emmeans(model, pairwise ~  plant_pathway_induced)})
  
simp.df <- lapply(simp.emmeans, function(emmeans){
  emmeans$contrasts %>% summary(infer=TRUE) %>% as.data.frame()})

#fix the df 
simp.df$cond_phytohormones$Experiment <- "Phytohormones"
simp.df$cond_real_insects$Experiment <- "Insect hervibory"

simp.df <-rbind(simp.df$cond_phytohormones,simp.df$cond_real_insects)#merge it
simp.df$Index <- "Chao1" #add index name
simp.df <- simp.df %>% relocate(Experiment, Index)#place experiment and index first
simp.df$p.value <- scales::pvalue(simp.df$p.value)#format p value
simp.df
...
chao_emm_both <- make_emmeans_table_2_experiments(chao.df, Chao1)
...
#Finally, merge them
alpha_posthoc_df <- rbind(obs_emm_both, chao_emm_both, shann_emm_both, pd_emm_both)
