...
ps_16S_split <- lapply(ps_16S_split, function(ps){
  subset_samples(ps, sample_or_blank != "blank") }) #43,45,48 left
...
Now, independently by experiment
```{r }


#as they come from independent experiments, separate them to filter them independently
ps_16S_split <- phyloseq_sep_variable(ps_16S, variable="Experiment", drop_zeroes = T)

#see how many reads
median <- lapply(ps_16S_split, function(ps){
         median(ps@sam_data$libsize_after_plastid)}) #87,821 | 60,108 | 83,092 reads 
  
mean <- lapply(ps_16S_split, function(ps){
         mean(ps@sam_data$libsize_after_plastid)}) #111,779 | 61,805 | 81,678 reads   

stdev <- lapply(ps_16S_split, function(ps){
         sd(ps@sam_data$libsize_after_plastid)}) #58,788 | 25,936 | 18,369 reads   
    

#which will be the maximum of reads allowed if we keep mean +/- 2 stdev?
max <- mapply(function(x,y){x+2*y}, x=median, y=stdev)
min <- mapply(function(x,y){x-2*y}, x=median, y=stdev)
...
#Phyto
max[1]
min[1] #no lower bound
sort(sample_sums(ps_16S_split$cond_phytohormones))
ps_16S_split$cond_phytohormones <- subset_samples(ps_16S_split$cond_phytohormones,
                                                  libsize_after_plastid < 210000) #6 samples lost 

#Insects
max[2] #almost no upper bound, will keep it
min[2] #~10,000
sort(sample_sums(ps_16S_split$cond_real_insects))
ps_16S_split$cond_real_insects <- subset_samples(ps_16S_split$cond_real_insects,
                                                  libsize_after_plastid > 8235) #2 samples lost 

#EPG
max[3] #~120,000
min[3] #~46,352
sort(sample_sums(ps_16S_split$EPG))
ps_16S_split$EPG <- subset_samples(ps_16S_split$EPG,
                                                  libsize_after_plastid > 46352) #1 sample lost, blank 
...
#remove ASVs with low-length sequences
#check final sequence length. 16S sequences smaller than 380bp should be discarded
lapply(ps_16S_split, function(ps){summary(ps@refseq@ranges@width)})

#remove the ones smaller than 380bp
ps_16S_split <- lapply(ps_16S_split, function(ps){
  prune_taxa(taxa = as.data.frame(refseq(ps)@ranges)$width>380, x = ps)})
...
#add column to sample_data
ps_16S_split <- lapply(ps_16S_split, function(ps){
  ps@sam_data$libsize_after_filtering <- sample_sums(ps)
  return(ps)
})

#check how many reads were kept at the end
ps_16S_split <- lapply(ps_16S_split, function(ps){
  ps@sam_data$pct_reads_kept <- ((ps@sam_data$libsize_after_filtering/ps@sam_data$total_reads)*100)
  return(ps)
})
...
rm.all.but(keep=c("ps_16S", "ps_16S_split"), keep_functions = TRUE)
...
I consider losing between 20% and 24% of the reads seems like a lot, perhaps ITS data is different than 16S where only 0.7% of the reads were lost when using a combination of prevalence and frequency.
...
#ps_ITS_filtering_finished <- ps_ITS #save it to be used when split by experiment
...
ps_phylum %>%
refseq() %>%
Biostrings::writeXStringSet("./amplicon_data//ITS_phylum_NA.fna",
...
#clean
rm( number, ps_phylum, ps_sel, x, y, z, f_ASV1, f_ASV27, f_ASV80, f_ASV81, f_ASV95,f_ASV146,
   tax,tax_df,ASV_sel, colnames, Phylum_NA)

```


#FunGuild[not working on August 2nd, 2024]
...
sums[order(sample_sums(ps_ITS))] #652 to 179,000
...
#which will be the maximum of reads allowed if we keep mean +/- 1 stdev?
mean + 1*stdev #148,428.8 ~ 150,000
mean - 1*stdev #55,653 ~ 55,000
...
ps_ITS <- subset_samples(ps_ITS, libsize_after_nonFungi > 50000) 
ps_ITS <- subset_samples(ps_ITS, libsize_after_nonFungi < 170000) #120 samples left, increasing to 170,000 
...
ps_ITS_filtering_finished
ps_ITS_filtering_finished@sam_data$libsize_after_nonFungi

#as they come from independent experiments, separate them to filter them independently
ps_ITS_split <- phyloseq_sep_variable(ps_ITS_filtering_finished, variable="Experiment", drop_zeroes = T)

#see how many reads
median <- lapply(ps_ITS_split, function(ps){
         median(ps@sam_data$libsize_after_nonFungi)}) #87,821 | 60,108 | 83,092 reads 
  
mean <- lapply(ps_ITS_split, function(ps){
         mean(ps@sam_data$libsize_after_nonFungi)}) #111,779 | 61,805 | 81,678 reads   

stdev <- lapply(ps_ITS_split, function(ps){
         sd(ps@sam_data$libsize_after_nonFungi)}) #58,788 | 25,936 | 18,369 reads   
...
max[1]
min[1] #no lower bound
sort(sample_sums(ps_ITS_split$cond_phytohormones)) #however reads are very low, 49samples
ps_ITS_split$cond_phytohormones <- subset_samples(ps_ITS_split$cond_phytohormones,#use less than 10,000 reads
                                                  libsize_after_nonFungi > 10000) #18 samples lost

#Insects
max[2] #almost no upper bound, will keep it
min[2] #~44,693
sort(sample_sums(ps_ITS_split$cond_real_insects)) #48samples
ps_ITS_split$cond_real_insects <- subset_samples(ps_ITS_split$cond_real_insects,#46 samples
                                                  libsize_after_nonFungi > 45000) #2 samples lost 

#EPG
max[3] #158,728 ~160,000
min[3] #~92,228.87
sort(sample_sums(ps_ITS_split$EPG))
ps_ITS_split$EPG <- subset_samples(ps_ITS_split$EPG,
                                    libsize_after_nonFungi  < 160000) #2 samples lost 

#remove blanks
ps_ITS_split <- lapply(ps_ITS_split, function(ps){
  subset_samples(ps, sample_or_blank != "blank") }) #43,45,48 left

#remove ASVs with low-length sequences
#check final sequence length. 16S sequences smaller than 380bp should be discarded
lapply(ps_ITS_split, function(ps){summary(ps@refseq@ranges@width)})

#remove the ones smaller than 100bp
ps_ITS_split <- lapply(ps_ITS_split, function(ps){
  prune_taxa(taxa = as.data.frame(refseq(ps)@ranges)$width>100, x = ps)})
