...
#Save tables 
. <- c("Soil inocula (S)", "Herbivory (H)", "S*H", "Residuals")
lm_table <- cbind (., as.data.frame(Anova(lm)))
lm_table$`Pr(>F)` <- round(lm_table$`Pr(>F)`, 3)
table <- nice_table(lm_table,
              title = c("Table X. Linear Model for plant biomass in Feedback phase - Phyto"),
...
lm.i <- lm(shoot_biomass_mg ~ soil_inocula*herbivory,
         data=insects)
...
table <- nice_table(lm.i_table,
              title = c("Table X. Linear Model for plant biomass in Feedback phase - Insects"),
              note = "shoot_biomass_mg  ~ soil_inocula*herbivory")
