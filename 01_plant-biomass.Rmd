...
lm_table$`Pr(>Chisq)` <- round(lm_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm_table,
              title = c("Table X. GLM for plant biomass in Conditioning phase - Phyto"),
              note = "shoot_biomass_mg  ~ treatment, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/01_plant-biomass/conditioning_lm_biomass_phyto.docx")

###

#post-hoc test
emmeans <- emmeans(aov_simulated, "plant_pathway_induced", data=simu)
posthoc <- cld(emmeans, letters="letters", adjust="tukey")
...
#quick summary
real %>% group_by(Experiment, plant_pathway_induced) %>% summarise(n=n())

aov_real <- glm(shoot_biomass_mg  ~ plant_pathway_induced,
                family=gaussian,
                   data=real)

lm.anova <- car::Anova(aov_real, type="II", test.statistic="LR")

simulateResiduals(aov_real, plot=T) #ok
...
lm_table$`Pr(>Chisq)` <- round(lm_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm_table,
              title = c("Table X. GLM for plant biomass in Conditioning phase - Insects"),
              note = "shoot_biomass_mg  ~ treatment, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/01_plant-biomass/conditioning_lm_biomass_insects.docx")
...
lm.i.anova <- car::Anova(lm.i, type="II", test.statistic="LR")
