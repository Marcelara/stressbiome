...
#Phyto
phyto.i <- subset(long_cond, Experiment == "simulated" & glucosinolate == "Glu_alyphatic")
...
lm_table <- car::Anova(lm, type="II", test.statistic="LR")
simulateResiduals(lm, plot=T) #ok

#save result
. <- c("Treatment(T)", "Glucosinolate(G)", "T*G")
lm_table <- cbind (., as.data.frame(lm_table))
lm_table$`Pr(>Chisq)` <- round(lm_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm_table,
              title = c("Table X. GLM for GLS in conditioning phase - Phyto"),
              note = "~ Treatment*glucosinolate type, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/conditioning_phyto.docx")

#emeans
emmeans <- emmeans(lm, ~plant_pathway_induced)
...
#Insects
#Phyto
insects <- subset(long_cond, Experiment == "real_insects" & glucosinolate == "Glu_indolic")
insects %>% group_by(plant_pathway_induced) %>% summarise(n=n()) #balanced N=4
...
lm_table$`Pr(>Chisq)` <- round(lm_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm_table,
              title = c("Table X. GLM for GLS in conditioning phase - Insects"),
              note = "~ Treatment*glucosinolate type, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/conditioning_insects.docx")
...
phyto %>% group_by(Experiment, plant_pathway_induced) %>% summarise(n=n()) #balanced N=8

lm <- glm(log(concentration) ~ plant_pathway_induced*glucosinolate,
          family=gaussian,
         data=phyto)
...
#save result
factors <- c("Soil inoculum (S)", "Herbivory (H)", "S*H")
lm.p.a_table <- cbind (factors, as.data.frame(Anova(lm.p.a_table)))
lm.p.a_table$`Pr(>Chisq)` <- round(lm.p.a_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm.p.a_table,
              title = c("Table X. GLM for aliphatic GLS in Feedback phase - Phyto"),
              note = "shoot_biomass_mg  ~ soil_inocula*herbivory, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/feedback_aliGLS_phyto.docx")
...
#pairiwse differences
emmeans_soil <- emmeans(lm.p.i, pairwise~soil_inocula) #differences between Ctrl and SA

#save result
factors <- c("Soil inoculum (S)", "Herbivory (H)", "S*H")
lm.p.i_table <- cbind (factors, as.data.frame(lm.p.i_table))
lm.p.i_table$`Pr(>Chisq)` <- round(lm.p.i_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm.p.i_table,
              title = c("Table X. GLM for indolic GLS in Feedback phase - Phyto"),
              note = "~ soil_inocula*herbivory")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/feedback_indoGLS_phyto.docx")

```

For herbivore-infested experiment 
```{r}
#---- Insects-Alyphatic ----
...
#save result
factors <- c("Soil inoculum (S)", "Herbivory (H)", "S*H")
lm.i.a_table <- cbind (factors, as.data.frame(lm.i.a_table))
lm.i.a_table$`Pr(>Chisq)` <- round(lm.i.a_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm.i.a_table,
              title = c("Table X. GLM for aliphatic GLS in Feedback phase - Insects"),
              note = "shoot_biomass_mg  ~ soil_inocula*herbivory, GLM-Gaussian")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/feedback_aliGLS_insects.docx")


#pairiwse differences
emmeans_soil <- emmeans(lm.i.a, pairwise~soil_inocula) #differences between CTR and JA and SA
emmeans_herb <- emmeans(lm.i.a, pairwise~herbivory)
...
#pairiwse differences
emmeans_herb <- emmeans(lm.i.i, pairwise~herbivory) #differences between Ctrl and SA

#save result
factors <- c("Soil inoculum (S)", "Herbivory (H)", "S*H")
lm.i.i_table <- cbind (factors, as.data.frame(lm.i.i_table))
lm.i.i_table$`Pr(>Chisq)` <- round(lm.i.i_table$`Pr(>Chisq)`, 3)
table <- nice_table(lm.i.i_table,
              title = c("Table X. GLM for indolic GLS in Feedback phase - Insects"),
              note = "~ soil_inocula*herbivory")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/03_glucosinolates/feedback_indoGLS_insects.docx")
