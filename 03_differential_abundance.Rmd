...
## By treatment
#JA
JA <- df_Treatment2 %>% dplyr::select(id, DA_JA_phyto, DA_JA_insects)
AF2.1 <- itol.toolkit::create_unit(data = JA, 
                         key = "AF2.1_Treatment_JA", 
                         type = "DATASET_BINARY",
                         tree = Bac_tree)

AF2.1@field$labels <- c("JA_phyto",  "JA_insects")
AF2.1@field$colors <- c("#E69F00",  "#E69F00")

itol.toolkit::write_unit(AF2.1, "./Results/03_differential_abundance/16S_iTOL")

#SA
SA <- df_Treatment2 %>% dplyr::select(id, DA_SA_phyto, DA_SA_insects)
AF2.2 <- itol.toolkit::create_unit(data = SA, 
                         key = "AF2.2_Treatment_SA", 
                         type = "DATASET_BINARY",
                         tree = Bac_tree)

AF2.2@field$labels <- c("SA_phyto",  "SA_insects")
AF2.2@field$colors <- c("#4472C4", "#4472C4")
...
JA <- df_LFC %>% dplyr::select(id, lfc_JA_phyto, lfc_JA_insects)

#get Annotation File for Heatmap
AF4.1 <- itol.toolkit::create_unit(data = JA , 
                         key = "AF4_LFC_Heatmap_JA", 
...
#select colors 
AF4.1@specific_themes$heatmap$color$min <- "#295e10"
AF4.1@specific_themes$heatmap$color$max <- "#590958"
...
AF4.1@specific_themes$heatmap$tree$tree_display <- 0

itol.toolkit::write_unit(AF4.1, "./Results/03_differential_abundance/16S_iTOL") #works

#SA
SA <- df_LFC %>% dplyr::select(id, lfc_SA_phyto, lfc_SA_insects)

#get Annotation File for Heatmap
AF4.2 <- itol.toolkit::create_unit(data = SA , 
                         key = "AF4.2_LFC_Heatmap_SA", 
                         type = "DATASET_HEATMAP",
                         tree = Bac_tree)  #works
...
#select colors 
AF4.2@specific_themes$heatmap$color$min <- "#295e10"
AF4.2@specific_themes$heatmap$color$max <- "#590958"
...
AF4.2@specific_themes$heatmap$tree$tree_display <- 0

itol.toolkit::write_unit(AF4.2, "./Results/03_differential_abundance/16S_iTOL") #works
...
Phylum <- unique(df_Phylum$Phylum) #10
colourCount <- 10
getPalette <- colorRampPalette(brewer.pal(colourCount, "Pastel2"))
color_phylum <- getPalette(colourCount)
color_phylum <- c("#FBB4AE", "#BBCADD", "#C6E4CB", "#D8D5D9", "#EFD2C1", "#FEE9B6", "#F6F2C7", "#EAD8C7", "#FBDCEC" ,"#F2F2F2")
...
itol.toolkit::write_unit(AF7, "./Results/03_differential_abundance/16S_iTOL")
```
...
df_experiment <- distinct(df_experiment, id, .keep_all = T) #keep unique ASVs
order_asvs <- df_experiment$id
...
#add column with Experiment including Phyto, Insects and both
df_complete <- df_complete %>% mutate(Experiment=df_experiment$Experiment_2)
...
AF1@common_themes$legend$title <- "Experiment"
AF1@common_themes$legend$horizontal <- 0
AF1@common_themes$legend$labels <- c("Both", "Herbivore-induced","Phytohormone-induced")
...
AF3@field$labels <- c("JA_phyto",  "JA_insects", "SA_phyto", "SA_insects")
AF3@field$colors <- c("#E69F00",  "#E69F00","#4472C4", "#4472C4")
...
JA <- df_Treatment2 %>% dplyr::select(id, DA_JA_phyto, DA_JA_insects)
AF3.1 <- itol.toolkit::create_unit(data = JA, 
                         key = "AF3.1_Treatment_JA", 
                         type = "DATASET_BINARY",
                         tree = ITS_tree)

AF3.1@field$labels <- c("JA_phyto",  "JA_insects")
AF3.1@field$colors <- c("#E69F00",  "#E69F00")

itol.toolkit::write_unit(AF3.1, "./Results/03_differential_abundance/ITS_iTOL")

#SA
SA <- df_Treatment2 %>% dplyr::select(id, DA_SA_phyto, DA_SA_insects)
AF3.2 <- itol.toolkit::create_unit(data = SA, 
                         key = "AF3.2_Treatment_SA", 
                         type = "DATASET_BINARY",
                         tree = ITS_tree)

AF3.2@field$labels <- c("SA_phyto",  "SA_insects")
AF3.2@field$colors <- c("#4472C4", "#4472C4")
...
#get Annotation File for Heatmap
AF4 <- itol.toolkit::create_unit(data = df_LFC , 
                         key = "AF4_LFC_Heatmap", 
                         type = "DATASET_HEATMAP",
                         tree = ITS_tree)  #works

#select colors 
AF4@specific_themes$heatmap$color$min <- "#295e10"
AF4@specific_themes$heatmap$color$max <- "#590958"
AF4@specific_themes$heatmap$color$mid <- "#ebeceb"
AF4@specific_themes$heatmap$value$mid <- 0
AF4@specific_themes$heatmap$color$nan <- "#FFFFFF"

AF4@specific_themes$heatmap$tree$tree_display <- 0
...
itol.toolkit::write_unit(AF4.1, "./Results/03_differential_abundance/ITS_iTOL") #works
...
itol.toolkit::write_unit(AF4.2, "./Results/03_differential_abundance/ITS_iTOL") #works
...
AF8 <- itol.toolkit::create_unit(data = df_trophic, 
                         key = "AF8_Trophic_mode", 
                         type = "DATASET_SYMBOL",
                         tree = ITS_tree)

AF8@data$tip$`AF10_Trophic_mode$POSITION`<- 0.8
