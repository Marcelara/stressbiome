...
ancom_16S_new <- lapply(ps_16S_split, function(ps){
  set.seed(123)
...
load("C:/RProjects/stressbiome/R_Environments/03_differential_abundance/16S_iTOL.Rdata")

phyto_dataDA$binary_stress #phyto
insects_dataDA$binary_stress #insects

#phyto:
JAp <- sum(phyto_dataDA$binary_stress$plant_pathway_inducedJA) #DA JA
SAp <- sum(phyto_dataDA$binary_stress$plant_pathway_inducedSA) #DA SA
Bothp <- sum(phyto_dataDA$binary_stress$plant_pathway_inducedJA & phyto_dataDA$binary_stress$plant_pathway_inducedSA) #how many are shared
Totalp <- dim(phyto_dataDA$binary_stress)[1] #total number 

#insects
JAi <- sum(insects_dataDA$binary_stress$plant_pathway_inducedJA) #DA JA
SAi <- sum(insects_dataDA$binary_stress$plant_pathway_inducedSA) #DA SA
Bothi <- sum(insects_dataDA$binary_stress$plant_pathway_inducedJA & insects_dataDA$binary_stress$plant_pathway_inducedSA) #how many are shared
Totali <- dim(insects_dataDA$binary_stress)[1] #total number 
...
#prepare df
fit <- euler(df_euler, control=list(shape="ellipse", loss="raw"))


#plot euler diagram & save it as svg
svg("./Plots/03_differential_abundance/euler_diagram_16S.svg",
    width = 8, height = 6)  # Set the file name and dimensions

plot(fit, 
     fills = list(fill = c(color_JA, color_SA, color_JA, color_SA), alpha = 0.9),# Set colors 
     edges = TRUE,  # Show edges of the circles
     labels = list(font = 4), # Customize labels
     quantities = TRUE,
     main = "DA ASVs 16S") # Add a title
...
min <- color_heatmap[8] "#D6604D"
max <- color_heatmap[1] "#4393C3"
...
ps_ITS_filt <- lapply(ps_ITS_split, function(ps){
  metagMisc::phyloseq_filter_prevalence(ps, prev.trh = 0.10, #at least 10% of samples
                                          abund.trh = NULL) #no filtering by abundance
})
...
# Extract set sizes and calculate percentages
set_sizes <- fit$original
total_size <- sum(set_sizes)
percentages <- round((set_sizes / total_size) * 100, 1)




#plot euler diagram & save it as svg
svg("./Plots/03_differential_abundance/euler_diagram_ITS.svg",
    width = 8, height = 6)  # Set the file name and dimensions
...
ps_phylum_NA  %>%
refseq() %>%
Biostrings::writeXStringSet("./Results/03_differential_abundance/ITS_phylum_NA_sel.fna",
             append=FALSE,compress=FALSE,
             compression_level=NA, format="fasta")

#fASV_238: More hits in Glomeromycetes; phylum Glomeromycota
#fASV_221: All of the hits in Ascomycete fungi; phylum Ascomycota

#now, replace in tax table
tax_table(ps_DA)[1,2] <- "Glomeromycota"
tax_table(ps_DA)[12,2] <- "Ascomycota"
...
AF5 <- create_unit(data = df_labels_G2,
                       key = "AF5_label_Genus",
                       type = "LABELS",
                       tree = ITS_tree)
...
AF7@data$tip$`AF7_Trophic_mode$POSITION`<- 1
AF7@data$tip$`AF7_Trophic_mode$SYMBOL` <- 2


itol.toolkit::write_unit(AF7, "./Results/03_differential_abundance/ITS_iTOL")


#colors: "#2ca02c", "#66c2a5", "#5ab4ac", "#fdae61", "#f46d43", "#d73027"
...
save.image("./R_environments/03_differential_abundance/ITS_iTOL.Rdata")
