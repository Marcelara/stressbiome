...
lm.p_table <- car::Anova(lm.p, type="II", test.statistic="LR")
simulateResiduals(lm.p, plot=T) #ok

#save result
factors <- c("Soil inocula(S)")
lm.p_table <- cbind (factors, as.data.frame(lm.p_table))
...
flextable::save_as_docx(table, path = "./Tables/04_insect_performance/caterpillar_weight_phyto.docx")
...
lm.i_table <- car::Anova(lm.i, type="II", test.statistic="LR")
simulateResiduals(lm.i, plot=T) #ok

#save result
factors <- c("Soil inocula(S)")
lm.i_table <- cbind (factors, as.data.frame(lm.i_table))
...
flextable::save_as_docx(table, path = "./Tables/04_insect_performance/caterpillar_weight_insects.docx")
...
phyto %>% group_by(Experiment,soil_inocula) %>% summarise(n=n()) #balanced
...
#get summary
x <- car::Anova(glm.p , type="II", test.statistic="LR")
x <- as.data.frame(x)

#save result
factors <- c("Soil inocula (S)", "Time point(T)", "S*T")
glm.p_table <- cbind (factors, x)
glm.p_table$`Pr(>Chisq)` <- round(glm.p_table$`Pr(>Chisq)`, 3)
...
flextable::save_as_docx(table, path = "./Tables/04_insect_performance/aphid_number_phyto.docx")


#---- insects ----
insects <- subset(aphids_long, Experiment == "real_insects")
insects %>% group_by(Experiment,soil_inocula) %>% summarise(n=n()) #not-balanced
...
#get summary
y <- car::Anova(glm.i , type="II", test.statistic="LR")
y <- as.data.frame(y)

#save result
factors <- c("Soil inoculum (S)", "Time point(T)", "S*T")
glm.i_table <- cbind (factors, y)
glm.i_table$`Pr(>Chisq)` <- round(glm.i_table$`Pr(>Chisq)`, 3)
...
flextable::save_as_docx(table, path = "./Tables/04_insect_performance/aphid_number_insects.docx")
