...
color_ctrl_vs_SA <- c("#BFBFBF",  "#319B67")
...
#make table for Word
. <- c("Experiment (E)", "Treatment (T)", "E*T", "Residual", "Total") #names of factors
permanova_table <- cbind (.,as.data.frame(permanova))

permanova_table$`Pr(>F)` <- round(permanova_table$`Pr(>F)`, 3)
table <- nice_table(permanova_table,
              title = c("PERMANOVA for Plant Phytohormones", "Conditioning phase"),
              note = "~Experiment*plant_pathway_induced, 999 number of permutations")

table <- table %>%
         flextable::colformat_num(big.mark = "", digits = 3) 

flextable::save_as_docx(table, path = "./Tables/02_phytohormones/conditioning_permanova_phytohormones.docx")
```
...
pairwise.adonis2 <- function(x, factors, sim.method = "euclidean", p.adjust.m = "bonferroni") {
...
. <- c("Experiment(E)", "Soil inocula(S)", "Herbivory(H)", "Residual", "Total") #names of factors
...
#save it 
flextable::save_as_docx(table, path = "./Tables/02_phytohormones/feedback_permanova_phytohormones.docx")
