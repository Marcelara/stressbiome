...
#83% phyto with final 14,999.47 reads ~15,000
...
#89% insects with final 31,770.55 reads ~31,771
...
#Jaccard
jacc <- lapply(ITS_CSS, function(ps){ distance(ps, "jaccard")})

#Unifrac
uni <- lapply(ITS_CSS, function(ps){ distance(ps, "unifrac")})

#Weighted Unifrac
wuni <- lapply(ITS_CSS, function(ps){ distance(ps, "wunifrac")})
...
dist <- list(bray=bray$cond_phytohormones, jacc=jacc$cond_phytohormones,
             uni=uni$cond_phytohormones, wuni=wuni$cond_phytohormones)
...
s.jacc <- merge(df_NMDS$jacc,centroids$jacc, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))
s.uni <- merge(df_NMDS$uni,centroids$uni, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))
s.wuni <- merge(df_NMDS$wuni,centroids$wuni, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))

segments <- list(bray=s.bray, jacc=s.jacc, uni=s.uni, wuni=s.wuni)
rm(s.bray, s.jacc, s.uni, s.wuni)#clean
...
p.Jacc <- plot_NMDS(df_NMDS$jacc, centroids$jacc, segments$jacc, "Jaccard")
p.uni <- plot_NMDS(df_NMDS$uni, centroids$uni, segments$uni, "Unifrac")
p.wuni <- plot_NMDS(df_NMDS$wuni, centroids$wuni, segments$wuni, "Weighted Unifrac")

#Make it a single figure 
ggarrange(p.Bray,p.Jacc,p.uni,p.wuni, ncol=4, nrow=1,common.legend = TRUE,legend="bottom")
...
s.bray <- merge(df_UMAP$bray,centroids_UMAP$bray, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))
s.jacc <- merge(df_UMAP$jacc,centroids_UMAP$jacc, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))
s.uni <- merge(df_UMAP$uni,centroids_UMAP$uni, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))
s.wuni <- merge(df_UMAP$wuni,centroids_UMAP$wuni, by.x="plant_pathway_induced", by.y="plant_pathway_induced",
        suffixes=c("","_centroid"))

segments_UMAP <- list(bray=s.bray, jacc=s.jacc, uni=s.uni, wuni=s.wuni)
rm(s.bray, s.jacc, s.uni, s.wuni)#clean
...
u.Jacc <- plot_UMAP(df_UMAP$jacc, centroids_UMAP$jacc, segments_UMAP$jacc, "Jaccard")
u.uni <- plot_UMAP(df_UMAP$uni, centroids_UMAP$uni, segments_UMAP$uni, "Unifrac")
u.wuni <- plot_UMAP(df_UMAP$wuni, centroids_UMAP$wuni, segments_UMAP$wuni, "Weighted Unifrac")

#Make it a single figure 
ggarrange(u.Bray,u.Jacc,u.uni,u.wuni, ncol=4, nrow=1,common.legend = TRUE,legend="bottom")
...
dist <- list(bray=bray$cond_real_insects, jacc=jacc$cond_real_insects,
             uni=uni$cond_real_insects, wuni=wuni$cond_real_insects)
...
#clean a little
rm(u.Bray, u.Jacc, u.uni, u.wuni, p.Bray, p.Jacc, p.uni, p.wuni)#remove plots
rm(centroids, centroids_UMAP, segments, segments_UMAP, df_UMAP, df_NMDS)#df's
rm(dist, nmds_result, umap_scores, umap_df, segments_data,x, ITS_exp, nmds_df, nmds_plot, nmds_scores,
   umap)
