...
#' (4) if annotation on time=0, then wave 1 at time=0 is not added (lines 257)
#' (5) remove argument epgtable (by default its in output now)
#' (6) replaced new column assignment via outp$variables with cbind - because did not work anymore with  new R version
#' (7) also adjustment of column classes (at end defined now y$groups as factor)
#'
...
#' @export
epgrun <- function(loc, epgstart, epgstop, epgplot, epgbin, epgname)
{
  ...
  for (i in 1:length(loc)) {
    ...
    for (j in 1:dim(anaobj)[2]) {
      ...
      df1[,1] <- factor(df1[,1]) #NEW#
      if (min(df1[,3]) > 0){#NEW#
        df1 <- rbind(df1, c(levels(df1[,1]), 1, 0, NA)) # starting with NP
      }#NEW#
 ...
 ## Previous and next waveforms
  ...
  outp <- cbind(outp, colnames(y)[4:ncol(y)])#NEW
  colnames(outp)[ncol(outp)] <- "variable" #NEW
  ...
  # Add variable names as a first column
  outp <- cbind(outp, colnames(yocc)[2:ncol(yocc)])#NEW
  colnames(outp)[ncol(outp)] <- "variable" #NEW
  #outp$variable <- colnames(yocc)[2:ncol(yocc)] #removed
  outp <- outp[,c(ncol(outp),1:(ncol(outp)-1))]
  # Change class of data of all data columns
